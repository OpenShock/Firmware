...
* Completely reworked RF/RMT transmitter for improved timing accuracy and shocker reliability
* **Captive portal overhaul** with improved stability and responsiveness, and rebuilt UI on **Svelte 5 + shadcn-svelte** with **Tailwind CSS v4**
* **E-Stop handling** is now significantly more reliable with change detection and a re-arm grace period
* Updated **AssignLCG** integration to the new backend endpoint (old LCG override setting removed)
* **Wi-Fi RSSI telemetry** reported by the hub for connection health display in the OpenShock.app frontend
