// Base64Utils encode/decode (mbedtls-backed) - used for auth tokens etc.
#include "unity.h"
...
TEST_CASE("Base64 encode known vectors", "[util][base64]")
{
  ...
  TEST_ASSERT_TRUE(Base64Utils::Encode(std::span<const uint8_t>(man, 3), out));
  TEST_ASSERT_TRUE(out == "TWFu");

  // padding cases
  const uint8_t m[1] = {'M'};
  TEST_ASSERT_TRUE(Base64Utils::Encode(std::span<const uint8_t>(m, 1), out));
  TEST_ASSERT_TRUE(out == "TQ==");
  const uint8_t ma[2] = {'M', 'a'};
  TEST_ASSERT_TRUE(Base64Utils::Encode(std::span<const uint8_t>(ma, 2), out));
...
{
  TinyVec<uint8_t> out;
  TEST_ASSERT_TRUE(Base64Utils::Decode("TWFu", out));
...
{
  TinyVec<uint8_t> out;
  TEST_ASSERT_FALSE(Base64Utils::Decode("!!!!", out));  // invalid chars
}
...
TEST_CASE("Base64 round-trip", "[util][base64]")
{
  ...
  std::string encoded;
  TEST_ASSERT_TRUE(Base64Utils::Encode(std::span<const uint8_t>(data, 5), encoded));

  TinyVec<uint8_t> decoded;
  TEST_ASSERT_TRUE(Base64Utils::Decode(encoded, decoded));
