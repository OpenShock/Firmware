...
std::vector<rmt_data_t> ShockLink::Rmt::XlcEncoder::GetSequence(std::uint16_t transmitterId, std::uint8_t channelId, std::uint8_t method, std::uint8_t intensity) {
  std::uint64_t data
    = (std::uint64_t(transmitterId)    << 24)
    | (std::uint64_t(channelId & 0xF)  << 20)
    | (std::uint64_t(method    & 0xF)  << 16)
    | (std::uint64_t(intensity & 0xFF) <<  8);

  data |= ShockLink::Checksum::CRC8(data) & 0xFF;
