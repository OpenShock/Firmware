...
std::vector<rmt_data_t>
  OpenShock::Rmt::PetTrainerEncoder::GetSequence(std::uint16_t shockerId, std::uint8_t method, std::uint8_t intensity) {
  std::uint8_t methodBit      = (0x80 | (1 << (method - 1))) & 0xFF;
  std::uint8_t methodChecksum = 0xFF ^ ((1 << (8 - method)) | 1);
