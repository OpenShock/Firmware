...
const rmt_data_t startBit = {1400, 1, 800, 0};

const rmt_data_t oneBit = {800, 1, 300, 0};

const rmt_data_t zeroBit = {300, 1, 800, 0};
...
std::vector<rmt_data_t> to_rmt_data_xlc(const std::vector<uint8_t>& data) {
  ...
  for (int i = 0; i < 3; ++i) pulses.push_back(zeroBit);

  return pulses;
}

std::vector<rmt_data_t>
  ShockLink::XlcRmtControl::GetSequence(std::uint16_t shockerId, std::uint8_t method, std::uint8_t intensity) {
  std::vector<uint8_t> data = {(shockerId >> 8) & 0xFF, shockerId & 0xFF, method, intensity};
