...
bool OpenShock::CertificateUtils::GetHostCertificate(const char* host, std::vector<char>& pem) {
  ...
  if (client.connected() == 0) {
    OS_LOGE(TAG, "Failed to connect to host %s", host);
    return false;
  }

  OS_LOGD(TAG, "Connected to host %s, fetching certificate", host);

  const mbedtls_x509_crt* cert = client.getPeerCertificate();

  if (cert == nullptr) {
    OS_LOGE(TAG, "Certificate is null");
    return false;
  }
  ...
  if (retval != 0) {
    OS_LOGE(TAG, "Failed to write pem buffer: %d", retval);
    return false;
  }

  OS_LOGD(TAG, "Successfully fetched certificate from host %s", host);
