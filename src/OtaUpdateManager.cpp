...
static bool otaum_flash_app_partition(const esp_partition_t* partition, const char* remoteUrl, const uint8_t (&remoteHash)[32])
{
...
static bool otaum_flash_fs_partition(const esp_partition_t* parition, const char* remoteUrl, const uint8_t (&remoteHash)[32])
{
...
static void otaum_updatetask(void* arg)
{
  ...
  while (true) {
    ...
    if (!otaum_flash_fs_partition(filesystemPartition, release.filesystemBinaryUrl.c_str(), release.filesystemBinaryHash)) {
      otaum_restore_wdt_timeout();
      continue;
    }
    if (!otaum_flash_app_partition(appPartition, release.appBinaryUrl.c_str(), release.appBinaryHash)) {
      otaum_restore_wdt_timeout();
      continue;
    }
