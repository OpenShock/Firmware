...
std::vector<rmt_data_t> ShockLink::PetTrainerRmtControl::GetSequence(std::uint16_t shockerId, std::uint8_t method, std::uint8_t intensity)
{
  std::vector<uint8_t> data = {
      (0x80 | methodRemap.at(method)),
      (shockerId >> 8) & 0xFF,
      shockerId & 0xFF,
      intensity,
      methodChecksum.at(method)};
