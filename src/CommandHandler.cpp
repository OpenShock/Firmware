...
bool CommandHandler::Init()
{
  ...
  if (!OpenShock::IsValidOutputPin(txPin)) {
    if (!OpenShock::IsValidOutputPin(OpenShock::Constants::Gpio::RfTxPin)) {
      OS_LOGE(TAG, "Configured RF TX pin (%hhi) is invalid, and default pin (%hhi) is invalid. Unable to initialize RF transmitter", txPin, OpenShock::Constants::Gpio::RfTxPin);
    ...
    OS_LOGW(TAG, "Configured RF TX pin (%hhi) is invalid, using default pin (%hhi)", txPin, OpenShock::Constants::Gpio::RfTxPin);
    txPin = static_cast<gpio_num_t>(OpenShock::Constants::Gpio::RfTxPin);
