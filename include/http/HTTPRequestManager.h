...
namespace OpenShock::HTTP {
  ...
  Response<std::size_t> Download(const char* url, const std::map<String, String>& headers, GotContentLengthCallback contentLengthCallback, DownloadCallback downloadCallback, const std::vector<int>& acceptedCodes = {200}, int timeoutMs = 10'000);
  Response<std::string> GetString(const char* url, const std::map<String, String>& headers, const std::vector<int>& acceptedCodes = {200}, int timeoutMs = 10'000);

  template<typename T>
  Response<T> GetJSON(const char* url, const std::map<String, String>& headers, JsonParser<T> jsonParser, const std::vector<int>& acceptedCodes = {200}, int timeoutMs = 10'000)
  {
