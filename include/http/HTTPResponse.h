...
namespace OpenShock::HTTP {
  ...
  class HTTPResponse {
    ...
    HTTPResponse(esp_err_t error) : m_handle(nullptr), m_error(error) { }
    HTTPResponse(esp_http_client_handle_t handle, std::int64_t contentLength, bool isChunked) : m_handle(handle), m_contentLength(contentLength), m_isChunked(isChunked) {
      if (m_handle == nullptr) {
        m_error = ESP_FAIL;
        return;
      }
